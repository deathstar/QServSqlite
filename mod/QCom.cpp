...
namespace server {
    ...
    // Use a vector of strings to hold the IP addresses
    ::std::vector<::std::string> votedIPs;
    int mapsucksvotes = 0;
    QSERV_CALLBACK mapsucks_cmd(p) {
        clientinfo *ci = qs.getClient(CMD_SENDER);
    
        // 1. Check if this IP string is already in the list
        ...
        if(mapsucksvotes >= (maxclients / 2)) {
            ...
            // 3. Reset trackers for the next map
            mapsucksvotes = 0;
            votedIPs.clear();
                            ...
                            if(cn!=CMD_SENDER && cn >= 0 && cn <= 1000 && ci != NULL && ci->connected && args[1] != NULL) {
                                if(!ci->isSpecLocked) {
                                    ...
                                    for(auto it = lockedSpecIPs.begin(); it != lockedSpecIPs.end(); ) {
    									if(*it == ci->ip) it = lockedSpecIPs.erase(it);
    									else ++it;
									}
                        ...
                        if(ci->connected) {
                            ...
                            if(cn!=CMD_SENDER && cn >= 0 && cn <= 1000 && ci != NULL && ci->connected && args[1] != NULL) {
                                ...
                                for(auto it = editMutedIPs.begin(); it != editMutedIPs.end(); ) {
                                    if(*it == ci->ip) it = editMutedIPs.erase(it);
                                    else ++it;
                                }
