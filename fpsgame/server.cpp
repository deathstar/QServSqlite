...
namespace server {
    ...
    int get_ctf_category(int mode)
    {
        switch(mode)
        {
            case 11: return 1; // InstaCTF
            case 12: return 2; // EfficCTF
            case 17: return 3; // Standard CTF
            default: return 0; // Not a trackable CTF mode
        }
    }
	
	bool is_flagrun_match(const _flagrun &fr, int current_mode, const char *current_map)
	{
	    int cat = get_ctf_category(current_mode);
	    if(cat == 0) return false; 
	    
	    return (get_ctf_category(fr.gamemode) == cat) && (fr.map == current_map);
	}
	...
	void _loadflagruns()
	{
	    ...
	    while(f->getline(line, sizeof(line)))
	    {
	        ...
	        std::string header, mapname, playername;
	        int mode, timeused;
	
	        // 2. Validate header and extract fields
	        // Format: flagrun [mode] "[map]" [time] "[name]"
	        if (!(ss >> header >> mode)) continue; 
	        if (header != "flagrun") continue;
	
	        // 3. Extract mapname (handles potential spaces in quotes)
	        if (!std::getline(ss >> std::ws, mapname, '\"')) continue; // Skip initial quote
	        if (!std::getline(ss, mapname, '\"')) continue;          // Get map content
	
	        // 4. Extract time
	        if (!(ss >> timeused)) continue;
	
	        // 5. Extract playername
	        if (!std::getline(ss >> std::ws, playername, '\"')) continue;
	        if (!std::getline(ss, playername, '\"')) continue;
	
	        // 6. Final safety check: validate the data before pushing
	        if (mode < 0 || mode > 50 || timeused <= 0 || mapname.empty()) {
	            printf("DEBUG: Skipping malformed flagrun entry: %s\n", line);
	            continue;
	        }
	
	        _flagruns.push_back({mapname, mode, playername, timeused});
	    }
	    delete f;
	}
    ...
    void _doflagrun(clientinfo *ci, int timeused)
    {
        ...
        if(serverflagruns)
        {
            ...
            for(auto &f : _flagruns)
            {
                ...
                if(is_flagrun_match(f, gamemode, smapname))
                { 
                    fr = &f; 
                    break; 
                }
            ...
            if(!fr)
            {
                ...
                string cleanname;
				const char *src = ci->name;
				char *dst = cleanname;
				while(*src)
				{
				    if(*src == '\f') // Found a color code
				    {
				        src++; // Skip the '\f'
				        if(*src) src++; // Skip the color digit
				    }
				    else *dst++ = *src++;
				}
				*dst = '\0'; // Null-terminate the new string
				_flagruns.push_back({smapname, gamemode, cleanname, timeused});
                fr = &_flagruns.back();
                isbest = true;
            }
            else
            {
                ...
                if(isbest)
                {
                    ...
                    fr->gamemode = gamemode; //update record to current mode
                    printf("DEBUG: Record UPDATED/CREATED. Now: %s | Mode: %d | Time: %d\n", 
            		fr->name.c_str(), fr->gamemode, fr->timeused);
            		_storeflagruns();
                }
            }
    ...
    void _storeflagruns()
    {
        if(!serverflagruns) return;
    
        stream *f = openutf8file("config/flagruns.cfg", "w");
        if(f)
        {
            f->printf("//Automatically generated by QServ: lists best flagruns\n\n");
   ...
   void showflagrun(int cn)
    {
        extern clientinfo *getinfo(int n);
        
        auto local_concat = [](char *d, size_t bufsize, const char *fmt, ...) {
            va_list v;
            va_start(v, fmt);
            char *p = d + strlen(d);
            size_t remaining = (bufsize > (size_t)(p - d)) ? (bufsize - (p - d)) : 0;
            if (remaining > 0) vsnprintf(p, remaining, fmt, v);
            va_end(v);
        };
        ...
        if(smapname[0] == '\0') 
        {
            sendf(cn, 1, "ris", N_SERVMSG, "Error: No map loaded yet.");
            return;
        }
    
        _flagrun *maprecord = nullptr;
        _flagrun *personalbest = nullptr;
        
        string clean_ci_name;
        const char *src = ci->name;
        char *dst = clean_ci_name;
        while(*src)
        {
            if(*src == '\f') { src++; if(*src) src++; }
            else *dst++ = *src++;
        }
        *dst = '\0';
        ...
        for(auto &f : _flagruns)
        {
            if(is_flagrun_match(f, gamemode, smapname))
            {
                if(!maprecord || f.timeused < maprecord->timeused) 
                    maprecord = &f;
                
                if(strcasecmp(f.name.c_str(), clean_ci_name) == 0)
                {
                    if(!personalbest || f.timeused < personalbest->timeused)
                        personalbest = &f;
                }
            }
        }
        ...
        local_concat(msg, sizeof(msg), "Flagrun for \f0%s \f7(\f1%s\f7): ", smapname, modename(gamemode, "unknown"));
        
        if(maprecord)
        {
            local_concat(msg, sizeof(msg), "Best: \f0%s \f7(\f2%i.%02i \f7s in \f1%s\f7) ", 
                maprecord->name.c_str(), 
                maprecord->timeused/1000, (maprecord->timeused%1000)/10,
                modename(maprecord->gamemode, "unknown"));
        }
        else
        {
            local_concat(msg, sizeof(msg), "No best time yet. ");
        }
    
        if(personalbest && maprecord != personalbest)
        {
            local_concat(msg, sizeof(msg), "| Your PB: \f2%i.%02i \f7s", 
                personalbest->timeused/1000, (personalbest->timeused%1000)/10);
        }
        else if(personalbest && maprecord == personalbest)
        {
            local_concat(msg, sizeof(msg), "| \f2(You hold the record!)");
        }
    ...
    { 
        clientinfo *c = (clientinfo *)ci;
        
        if(c->authkickreason) 
        {
            delete[] c->authkickreason;
            c->authkickreason = NULL;
        }
    ...
    const char *blkmsg[] = {"fuck", "shit", "cunt"}; 
    bool is_blocked(const char *text) {    
        for (int i = 0; i < 3; i++) {
            if (strstr(text, blkmsg[i]) != NULL) {
                return true;
            }
        }
        return false;
    ...
    void textblk(char *text, clientinfo *ci) {
        if (is_blocked(text)) {
            int n = randomMT() % 7; 
    ...
    BOOL WINAPI ConsoleHandler(DWORD signal)
    {
        ...
        {
            printf("Windows shutdown signal received. Requesting graceful shutdown...\n");
            shutdown_requested = 1; 
            return TRUE;
        }
        return FALSE;
    ...
    void load_pbans(); void verifybans();
    void serverinit()
    {
        smapname[0] = '\0';
        resetitems();
...
#include "collect.h"
    ...
    void update_team_counters() {
    	...
    	team_evil_count = 0;
        loopv(clients) {
			if (!clients[i] || !clients[i]->team[0] || clients[i]->state.state == CS_SPECTATOR) {
				continue;
			}
 ...
 void senddemo(clientinfo *ci, int num, int tag)
    ...
    struct userinfo : userkey
    {
        ...
        userinfo() : userkey(), pubkey(NULL), privilege(PRIV_NONE) {} 
 ...
 int welcomepacket(packetbuf &p, clientinfo *ci)
   	...
   	void sendplayerstats(clientinfo *ci)
    {
        ...
        sqlite3 *db = qs.getDB();
        if(!db) {
            fprintf(stderr, "CRITICAL: Database connection lost. Stats lost for session.\n");
            return;
        }
	...
	//we init a new db handle for this command only to prevent crashing
	void cmd_sendplayerstats(int cn)
	{
	    ...
	    {   //mutex scope
	    	...
	    	// Save stats before showing if 30 seconds have passed
	    	if (totalmillis - ci->last_update_time_sp > 30000) 
	    	{
	        	if (ci) server::savestats(ci);
	        	ci->last_update_time_sp = totalmillis;
	    	}
	    }
	    ...
	    if(sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK)
	    {
	        ...
	        if(sqlite3_step(stmt) == SQLITE_ROW)
	        {
	            ...
	            float kd = (deaths > 0) ? (float)frags / (float)deaths : (float)frags;
	        ...
	        {
	            sendf(cn, 1, "ris", N_SERVMSG, "Stats not found.");
	        }
	    }
	    else {
	...
	void cmd_showLeaderboard(int cn)
	{	    
		clientinfo *ci = (clientinfo *)getclientinfo(cn);
	    if (!ci || !ci->connected || ci->state.aitype != AI_NONE || !ci->name[0]) return;
	    ...
	    // Save stats before showing if 30 seconds have passed
	    if (totalmillis - ci->last_update_time_lb > 30000) 
	    {
	        if (ci) server::savestats(ci);
	        ci->last_update_time_lb = totalmillis;
	    }
	
	    sqlite3 *db;
	    if(sqlite3_open("playerinfo.db", &db) != SQLITE_OK) return;
	
	    const char *sql = "SELECT NAME, FRAGS, DEATHS, FLAGS, KD FROM PLAYERINFO WHERE FRAGS > 0 ORDER BY KD DESC LIMIT 5;";
	    ...
	    if(sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK)
	    {
	        ...
	        while(sqlite3_step(stmt) == SQLITE_ROW)
	        {
	            ...
	            const char *name = name_raw ? (const char *)name_raw : "Unknown";
	            int frags  = sqlite3_column_int(stmt, 1);
	            ...
	            defformatstring(line)("%d. \f0%s\f7 - Frags: \f1%d\f7, Deaths: \f3%d\f7 (K/D: %.2f), Flags: \f5%d\f7", 
	                                  rank++, name, frags, deaths, kd, flags);
	            sendf(cn, 1, "ris", N_SERVMSG, line);
	        }
	        
	        if(!found) sendf(cn, 1, "ris", N_SERVMSG, "No stats available yet.");
	    }
...
best.add(clients[i]); \
    ...
    {
    	if(!actor || !target || !actor->connected || !target->connected || !target->state.o.x) return;
    ...
    void clearbanners() {
    	loopv(banners) delete[] banners[i];
    	banners.shrink(0); 
	}
   ...
   #include <string>
    ...
    void verifybans()
    {
        ...
        loopv(clients)
        {
            ...
            {
                to_disconnect.add(ci->clientnum);
            }
        }
    
        loopv(to_disconnect)
        {
            disconnect_client(to_disconnect[i], DISC_IPBAN);
        }
    }
    
    void save_pbans()
    {
        FILE *f = fopen("config/pbans.cfg", "w");
        if(!f) return;
        loopv(ipbans.bans)
        {
            fprintf(f, "%s\n", ipbans.bans[i].ipstr);
    ...
    void load_pbans()
    {
        FILE *f = fopen("config/pbans.cfg", "r");
        if(!f) return;
        char line[100];
    ...
    void sendkickbanlist(int cn)
    {
        ...
        loopv(bannedips)
        {
            ...
            n = snprintf(buf, sizeof(buf), "\f2id:\f7%2d\f2, ip: \f7", i);
            msgbuf.put(buf, n);
            n = im.print(buf);
            msgbuf.put(buf, n);
            n = snprintf(buf, sizeof(buf), "\f2, expires in: \f7");
            msgbuf.put(buf, n);
...
#include <time.h>
            ...
            case N_SWITCHMODEL:
            {
      		...
      		case N_SWITCHTEAM:
      		{
      		    ...
      		    if(m_teammode && text[0] && strcmp(ci->team, text) && 
            ...
            case N_SETTEAM:
            {
    			...
    			getstring(text, p);
   				text[MAXTEAMLEN] = '\0'; // Ensure it's null-terminated at the max team length
            ...
            case N_SETMASTER:
            {
                ...
                else setmaster(ci, val!=0, text); // don't broadcast the master password
                break;
            }
